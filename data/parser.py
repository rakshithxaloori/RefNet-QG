...
from remove_non_language import remove_non_language

# Generalize for dev, train from sys.argv
if len(sys.argv) != 5:
    sys.exit("Usage: python parser.py data_json_file data\'s_type language language_two_letter_code")
...
languageParsedDirectoryPath = os.getcwd() + os.sep + "parsedData" + os.sep + language + "_" + dataType
try:
...
if os.path.exists(os.path.join(languageParsedDirectoryPath, dataType + "_" + language + "_data.json")):
    os.remove(os.path.join(languageParsedDirectoryPath, dataType + "_" + language + "_data.json"))
...
for eachData in data["data"]:
    ...
    data_list.append(remove_non_language(copiedEachData["paragraphs"][0]["context"], language_code))

    contextFile = open(os.path.join(languageParsedDirectoryPath, dataType + "_context"), 'a+')
    questionFile = open(os.path.join(languageParsedDirectoryPath, dataType + "_question"), 'a+')
    answerFile = open(os.path.join(languageParsedDirectoryPath, dataType + "_answer"), 'a+')
    ...
    for paragraph in eachData["paragraphs"]:
        for qa in paragraph["qas"]:
            ...
            contextFile.write(remove_non_language(paragraph["context"], language_code))
            contextFile.write('\n')
